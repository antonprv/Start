...
{
    /// <summary>
    /// Fast quaternion math for System.Numerics.Quaternion.
    ///
    /// Hack pipeline used across operations:
    ///   • FastInvSqrt (hardware RSQRT estimate + Newton-Raphson) - normalization
    ///     and 1/sin(ω). See Core/SquareRootScalar.cs for why this replaced the
    ///     old Quake III bit-hack (faster AND more accurate on modern hardware -
    ///     benchmarked).
    ///   • sinSq * invSin trick              - sqrt(x) from a single FastInvSqrt call
    ///   • FastAtan2 (polynomial)            - ω = acos(dot) without Math.Acos
    ///   • 5th-order Horner sin poly (Core/Trigonometry.cs) - blend weights without Math.Sin
    ...
    /// No heap allocations occur when working with System.Numerics.Quaternion.
    ///
    /// Sin/cos: this file calls Core.FMath's shared FastSin/FastCos (safe, any
    /// finite input) or FastSinUnsafe/FastCosUnsafe (fast, PRECONDITION
    /// x ∈ [-π/2, π/2]) via `using static` - each call site below documents
    ...
    public static partial class FMath
    {
        ...
        /// Zero allocations - Vector3 is a value type.
        /// </summary>
        [MethodImpl( MethodImplOptions.AggressiveInlining )]
