// Regression test for the exact bug found during this session's review:
// FromEulerZYX/XYZ/YXZ used a half-angle sin/cos polynomial only valid for
// |half-angle| ≤ π/2, i.e. |angle| ≤ π - but nothing normalized or checked
// the input. Un-normalized/accumulated Euler angles (a common real-world
// case) silently produced an incorrect, non-unit quaternion.
...
namespace FastMath.Tests
{
    public class FromEulerTests
    {
        ...
        public void FromEulerZYX_AnyFiniteAngle_ProducesUnitQuaternion( float yaw, float pitch, float roll )
        {
            var q = FMath.FromEulerZYX( yaw, pitch, roll );
        ...
        [Test]
        public void FromEulerZYX_OutOfRangeYaw_MatchesExactFormula()
        {
        ...
        [Test]
        public void FromEulerXYZ_AnyFiniteAngle_ProducesUnitQuaternion()
        {
            var q = FMath.FromEulerXYZ( 5f, -6f, 8f );
            float len = MathF.Sqrt( q.X * q.X + q.Y * q.Y + q.Z * q.Z + q.W * q.W );
            Assert.That( len, Is.EqualTo( 1f ).Within( 0.01f ) );
        }

        [Test]
        public void FromEulerYXZ_AnyFiniteAngle_ProducesUnitQuaternion()
        {
            var q = FMath.FromEulerYXZ( 5f, -6f, 8f );
            float len = MathF.Sqrt( q.X * q.X + q.Y * q.Y + q.Z * q.Z + q.W * q.W );
            Assert.That( len, Is.EqualTo( 1f ).Within( 0.01f ) );
        }

        [Test]
        public void FromEulerZYX_Zero_IsIdentity()
        {
            var q = FMath.FromEulerZYX( 0f, 0f, 0f );
            Assert.That( FMath.IsIdentity( q ), Is.True );
        }
    }
}