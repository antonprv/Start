    ...
    public unsafe class BatchCompressor
    {
        ...
        public void Compress( BufferPool pool, IThreadDispatcher threadDispatcher = null, bool deterministic = false )
        {
            ...
            if ( constraintCount == 0 )
                return;
            var maximumCompressionCount = (int)FMath.Max( 1, FMath.Round( MaximumCompressionFraction * constraintCount ) );
            var targetCandidateCount = (int)FMath.Max( 1, FMath.Round( TargetCandidateFraction * constraintCount ) );
            ...
            for ( int i = 0; i < workerCount; ++i )
            {
                ...
                workerCompressions[ i ] = new QuickList<Compression>( FMath.Max( 8, maximumCompressionCount ), threadDispatcher == null ? pool : threadDispatcher.WorkerPools[ i ] );
            }
