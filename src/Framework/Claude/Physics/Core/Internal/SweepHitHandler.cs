...
namespace Framework.Physics.Internal
{
    ...
    internal struct SweepHitHandler : ISweepHitHandler
    {
        ...
        public bool AllowTest( CollidableReference collidable )
        {
            ...
            CollidableUserData other = _callbacks.GetData( collidable );

            // Triggers must be transparent to every sweep-based movement query (character
            // MoveAndSlide, projectile sweeps, ad-hoc SweepSphereCast) - they're only meant to be
            // detected via overlap events from the solver's narrow phase (ConfigureContactManifold),
            // never to physically block movement the way solid geometry does. Without this check,
            // a trigger with layer/mask permissive enough to generate overlap events (required for
            // it to be detected at all) was also permissive enough to stop a sweep dead, making
            // every trigger behave like an invisible wall.
            if ( other.Kind == PhysicsObjectKind.Trigger )
                return false;

            return _self.CanInteractWith( other );
        }
