...
namespace FastMath_Tests.Godot
{
	public class FromEulerTests
	{
		...
		{
			var q = GMath.FromEulerZYX( yaw, pitch, roll );
		...
		{
			var q = GMath.FromEulerXYZ( 5f, -6f, 8f );
			float len = MathF.Sqrt( q.X * q.X + q.Y * q.Y + q.Z * q.Z + q.W * q.W );
			Assert.That( len, Is.EqualTo( 1f ).Within( 0.01f ) );
		}
		...
		{
			var q = GMath.FromEulerYXZ( 5f, -6f, 8f );
			float len = MathF.Sqrt( q.X * q.X + q.Y * q.Y + q.Z * q.Z + q.W * q.W );
			Assert.That( len, Is.EqualTo( 1f ).Within( 0.01f ) );
		}
		...
		{
			var q = GMath.FromEulerZYX( 0f, 0f, 0f );
			Assert.That( GMath.IsIdentity( q ), Is.True );
		}
	}
}