...
using GMath = Framework.FastMath.Godot.FMath;

namespace FastMath_Tests.Godot
{
	public class SquareRootTests
	{
		...
		public void FastInvSqrt_Default_WithinHalfPercent( float x )
		{
			...
			float actual = GMath.FastInvSqrt( x );
		...
		{
			float expected = 1f / MathF.Sqrt( x );
			float defaultErr = MathF.Abs( GMath.FastInvSqrt( x, precise: false ) - expected );
			float preciseErr = MathF.Abs( GMath.FastInvSqrt( x, precise: true ) - expected );
		...
		{
			Assert.That( GMath.FastSqrt( 0f ), Is.EqualTo( 0f ) );
			Assert.That( GMath.FastSqrt( -5f ), Is.EqualTo( 0f ) );
		}
		...
		public void Normalize_Vector3_HugeComponent_NoNaN( float magnitude )
		{
			...
			GMath.Normalize( ref v );
			Assert.That( float.IsNaN( v.X ), Is.False );
			Assert.That( float.IsNaN( v.Y ), Is.False );
			Assert.That( float.IsNaN( v.Z ), Is.False );
			Assert.That( v.X, Is.EqualTo( 1f ).Within( SquareRootTestsHelpers.NORMALIZE_TOLERANCE ), $"expected ~(1,0,0), got ({v.X},{v.Y},{v.Z})" );
		}
		...
		public void Normalize_Vector3_HugeMixedComponents_NoNaN()
		{
			...
			Assert.That( len, Is.EqualTo( 1f ).Within( SquareRootTestsHelpers.NORMALIZE_TOLERANCE ) );
		}
		...
		public void Normalize_Vector2_HugeComponent_NoNaN( float magnitude )
		{
			...
			GMath.Normalize( ref v );
			Assert.That( float.IsNaN( v.X ), Is.False );
			Assert.That( v.X, Is.EqualTo( 1f ).Within( SquareRootTestsHelpers.NORMALIZE_TOLERANCE ) );
		}
		...
		public void Normalize_TypicalGameVector_UnaffectedByOverflowGuard()
		{
			...
			GMath.Normalize( ref v );
			Assert.That( v.X, Is.EqualTo( 0.6f ).Within( SquareRootTestsHelpers.NORMALIZE_TOLERANCE ) );
			Assert.That( v.Y, Is.EqualTo( 0.8f ).Within( SquareRootTestsHelpers.NORMALIZE_TOLERANCE ) );
		}
		...
		{
			Assert.That( GMath.IsNormalized( new Vector3( 1f, 0f, 0f ) ), Is.True );
		}

		[Test]
		public void IsNormalized_NonUnitVector_ReturnsFalse()
		{
			Assert.That( GMath.IsNormalized( new Vector3( 5f, 0f, 0f ) ), Is.False );
		}
	}
}