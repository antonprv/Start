...
namespace FastMath_Tests.Numerics
{
	public class SquareRootTests
	{
		...
		public void FastInvSqrt_Default_WithinHalfPercent( float x )
		{
			...
			float actual = NMath.FastInvSqrt( x );
		...
		{
			float expected = 1f / MathF.Sqrt( x );
			float defaultErr = MathF.Abs( NMath.FastInvSqrt( x, precise: false ) - expected );
			float preciseErr = MathF.Abs( NMath.FastInvSqrt( x, precise: true ) - expected );
		...
		{
			Assert.That( NMath.FastSqrt( 0f ), Is.EqualTo( 0f ) );
			Assert.That( NMath.FastSqrt( -5f ), Is.EqualTo( 0f ) );
		}
		...
		{
			Assert.That( NMath.IsNormalized( new Vector3( 1f, 0f, 0f ) ), Is.True );
		}

		[Test]
		public void IsNormalized_NonUnitVector_ReturnsFalse()
		{
			Assert.That( NMath.IsNormalized( new Vector3( 5f, 0f, 0f ) ), Is.False );
		}
	}
}