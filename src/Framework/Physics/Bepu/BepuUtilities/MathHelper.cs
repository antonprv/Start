...
namespace BepuUtilities
{
	...
	public static class MathHelper
	{
		...
		public static Vector<float> Cos( Vector<float> x )
		{
			...
			var twoPi = new Vector<float>( FMath.TWO_PI );
			...
			var pi = new Vector<float>( FMath.PI );
			...
			y = Vector.ConditionalSelect( Vector.GreaterThan( periodX, pi3Over2 ), new Vector<float>( FMath.TWO_PI ) - periodX, y );
		...
		public static Vector<float> Acos( Vector<float> x )
		{
			...
			var result = numerator / denominator;
			return Vector.ConditionalSelect( negativeInput, new Vector<float>( FMath.PI ) - result, result );
		}
		...
		public static void GetSignedAngleDifference( in Vector<float> a, in Vector<float> b, out Vector<float> difference )
		{
			...
			var x = ( b - a ) * new Vector<float>( 1f / FMath.TWO_PI ) + half;
			difference = ( x - Vector.Floor( x ) - half ) * new Vector<float>( FMath.TWO_PI );
		}
