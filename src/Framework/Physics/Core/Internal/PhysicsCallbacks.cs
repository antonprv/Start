...
namespace Framework.Physics.Internal
{
    ...
    internal struct BepuPoseIntegratorCallbacks : IPoseIntegratorCallbacks
    {
        ...
        {
            _linearDampingDt = new Vector<float>( MathF.Pow( FMath.Clamp( 1 - LinearDamping, 0, 1 ), dt ) );
            _angularDampingDt = new Vector<float>( MathF.Pow( FMath.Clamp( 1 - AngularDamping, 0, 1 ), dt ) );
    ...
    internal struct BepuNarrowPhaseCallbacks : INarrowPhaseCallbacks
    {
        ...
        public BepuNarrowPhaseCallbacks( CollidableProperty<CollidableUserData> properties, ContactPairSink sink, float frictionCoefficient, float maximumRecoveryVelocity )
        {
