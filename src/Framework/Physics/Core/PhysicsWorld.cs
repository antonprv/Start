...
namespace Framework.Physics
{
    ...
    public sealed class PhysicsWorld : IDisposable
    {
        ...
        private readonly Dictionary<uint, Func<float, BodyInertia>> _inertiaFactories = new Dictionary<uint, Func<float, BodyInertia>>();

        private Dictionary<ContactPairKey, (uint A, uint B)> _previousOverlaps = new Dictionary<ContactPairKey, (uint A, uint B)>();
        ...
        public PhysicsWorld( PhysicsWorldSettings settings )
        {
            ...
            _callbacks = new BepuNarrowPhaseCallbacks( _properties, _sink, settings.FrictionCoefficient, settings.MaximumRecoveryVelocity );
        ...
        public List<OverlapEvent> Step( float dt )
        {
            ...
            Dictionary<ContactPairKey, (uint A, uint B)> current = new Dictionary<ContactPairKey, (uint A, uint B)>( drained.Count );
        ...
        private OverlapEvent MakeEvent( (uint A, uint B) packed, bool entered )
        {
            ...
            return new OverlapEvent( _callbacks.GetData( refA ).OwnerId, _callbacks.GetData( refB ).OwnerId, entered );
        }
        ...
        public BodyHandle AddDynamicBody( PhysicsTransform pose, ShapeHandle shape, float mass, uint layer, uint mask, int ownerId,
            PhysicsObjectKind kind = PhysicsObjectKind.Solid, bool continuousDetection = false )
        {
            if ( !_inertiaFactories.TryGetValue( shape.Packed, out Func<float, BodyInertia>? inertiaFactory ) )
                throw new InvalidOperationException( "This shape does not support dynamic bodies (e.g. triangle meshes are static-only)." );

            TypedIndex typedIndex = new TypedIndex { Packed = shape.Packed };
            CollidableDescription collidable = new CollidableDescription( typedIndex, continuousDetection ? ContinuousDetection.Continuous() : ContinuousDetection.Discrete );
            RigidPose rigidPose = new RigidPose( pose.Position, pose.Orientation );
            BodyDescription description = BodyDescription.CreateDynamic( rigidPose, inertiaFactory( mass ), collidable, new BodyActivityDescription( 0.01f ) );

            BepuPhysics.BodyHandle handle = _simulation.Bodies.Add( description );
            _properties[ handle ] = new CollidableUserData { Layer = layer, Mask = mask, Kind = kind, OwnerId = ownerId };
            return new BodyHandle( handle.Value );
        }

        public BodyHandle AddKinematicBody( PhysicsTransform pose, ShapeHandle shape, uint layer, uint mask, int ownerId,
            PhysicsObjectKind kind = PhysicsObjectKind.Solid, bool continuousDetection = false )
        {
            TypedIndex typedIndex = new TypedIndex { Packed = shape.Packed };
            CollidableDescription collidable = new CollidableDescription( typedIndex, continuousDetection ? ContinuousDetection.Continuous() : ContinuousDetection.Discrete );
            RigidPose rigidPose = new RigidPose( pose.Position, pose.Orientation );
            BodyDescription description = BodyDescription.CreateKinematic( rigidPose, collidable, new BodyActivityDescription( 0.01f ) );
        ...
        public StaticHandle AddStatic( PhysicsTransform pose, ShapeHandle shape, uint layer, uint mask, int ownerId, PhysicsObjectKind kind = PhysicsObjectKind.Solid )
        {
            TypedIndex typedIndex = new TypedIndex { Packed = shape.Packed };
            RigidPose rigidPose = new RigidPose( pose.Position, pose.Orientation );
            BepuPhysics.StaticHandle handle = _simulation.Statics.Add( new StaticDescription( rigidPose, typedIndex ) );
            _properties[ handle ] = new CollidableUserData { Layer = layer, Mask = mask, Kind = kind, OwnerId = ownerId };
        ...
        {
            BepuPhysics.BodyHandle bepuHandle = new BepuPhysics.BodyHandle( handle.Value );
        ...
        {
            BepuPhysics.StaticHandle bepuHandle = new BepuPhysics.StaticHandle( handle.Value );
            if ( _simulation.Statics.StaticExists( bepuHandle ) )
                _simulation.Statics.Remove( bepuHandle );
        }

        public bool BodyExists( BodyHandle handle ) => _simulation.Bodies.BodyExists( new BepuPhysics.BodyHandle( handle.Value ) );

        public PhysicsTransform GetBodyPose( BodyHandle handle )
        {
            BodyReference reference = _simulation.Bodies.GetBodyReference( new BepuPhysics.BodyHandle( handle.Value ) );
        ...
        public Vector3 GetLinearVelocity( BodyHandle handle ) =>
            _simulation.Bodies.GetBodyReference( new BepuPhysics.BodyHandle( handle.Value ) ).Velocity.Linear;
        ...
        public void ApplyImpulse( BodyHandle handle, Vector3 impulse, Vector3 worldOffsetFromCenterOfMass )
        {
            BodyReference reference = _simulation.Bodies.GetBodyReference( new BepuPhysics.BodyHandle( handle.Value ) );
        ...
        /// </summary>
        public CharacterMoveResult MoveCharacter( BodyHandle self, Vector3 position, Vector3 velocity, float dt,
            float radius, float cylinderLength, uint layer, uint mask, CharacterMoveOptions options )
        {
            Capsule shape = new Capsule( radius, cylinderLength );
            float maxFloorDot = MathF.Cos( options.MaxFloorAngleDegrees * MathF.PI / 180f );
            ...
            {
                if ( displacement.LengthSquared() < 1e-10f )
                    break;
                ...
                float travelFraction = distance > 0f ? MathF.Max( 0f, hit.T - options.SkinWidth / distance ) : 0f;
                ...
                if ( Vector3.Dot( hit.Normal, Vector3.UnitY ) >= maxFloorDot )
                {
                    grounded = true;
            ...
            if ( !grounded )
            {
                SweepHitHandler probe = SweepOnce( shape, position, -Vector3.UnitY * options.FloorProbeDistance, layer, mask, self );
                if ( probe.Hit && Vector3.Dot( probe.Normal, Vector3.UnitY ) >= maxFloorDot )
                {
                    grounded = true;
        ...
        private SweepHitHandler SweepOnce( Capsule shape, Vector3 position, Vector3 displacement, uint layer, uint mask, BodyHandle? self )
        {
        ...
        /// </summary>
        public ProjectileSweepResult SweepProjectile( BodyHandle self, Vector3 position, Vector3 velocity, float dt, float radius, uint layer, uint mask )
        {
            Sphere shape = new Sphere( radius );
            Vector3 displacement = velocity * dt;
            ...
            if ( hitHandler.Hit )
            {
                ...
                return new ProjectileSweepResult( true, hitPosition, hitHandler.Point, hitHandler.Normal, hitHandler.HitOwnerId );
            }
        ...
        /// </summary>
        public ShapeCastResult SweepSphereCast( Vector3 origin, Vector3 direction, float maxDistance, float radius, uint layer, uint mask, BodyHandle? exclude = null )
        {
            if ( maxDistance <= 0f || direction.LengthSquared() < 1e-12f )
                return new ShapeCastResult( false, origin, default, Vector3.UnitY, 0f, 0 );

            Vector3 displacement = Vector3.Normalize( direction ) * maxDistance;
            ...
            if ( hitHandler.Hit )
            {
                ...
                return new ShapeCastResult( true, stopPosition, hitHandler.Point, hitHandler.Normal, distance, hitHandler.HitOwnerId );
            }

            return new ShapeCastResult( false, origin + displacement, default, Vector3.UnitY, maxDistance, 0 );
        }
